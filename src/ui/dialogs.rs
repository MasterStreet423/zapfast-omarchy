...
fn interactive_list(app: &mut App, ui: &mut egui::Ui, chat: &str, message: &str, button: usize) {
    ...
    ui.horizontal(|ui| {
        let choose = crate::i18n::gettext(app.locale, "Choose an option");
        let label = selected.map_or(&*choose, |button| button.label.as_str());
...
fn about(app: &mut App, ui: &mut egui::Ui) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.vertical(|ui| {
            ...
            theme::text(
                ui,
                crate::i18n::gettext(crate::i18n_extra::locale(), "Version {}")
...
fn confirm_delete_chat(app: &mut App, ui: &mut egui::Ui, id: &str) {
    let palette = app.palette;
    let name = app.chat(id).map_or_else(
        || crate::i18n::gettext(app.locale, "this chat").into_owned(),
...
fn join_group(app: &mut App, ui: &mut egui::Ui) {
    ...
    let members = crate::i18n::ngettext(app.locale, "{} member", "{} members", info.members as u32)
        .replace("{}", &info.members.to_string());
        ...
        ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
            ...
            let get_code = ui
                .add_enabled_ui(ready, |ui| {
                    ...
                    .on_disabled_hover_text(&*crate::i18n_extra::tr(NUMBER_TOO_SHORT))
                })
                .inner;
...
fn chat_info(app: &mut App, ui: &mut egui::Ui, id: &str) {
    ...
    if chat.is_group() && !chat.participants.is_empty() {
        ...
        theme::text(
            ui,
            crate::i18n::gettext(crate::i18n_extra::locale(), "Members ({})")
    ...
    if let Some(until) = chat.muted_until {
        theme::text(
            ...
            if until == 0 {
                ...
                crate::i18n::gettext(app.locale, "Muted until {}")
                    .replace("{}", &crate::util::chat_stamp(app.locale, until))
            },
            theme::regular(12.5),
            palette.secondary,
        );
    ...
    let mut buttons: Vec<(Icon, std::borrow::Cow<'static, str>, Vec<Action>)> = Vec::new();
